...
public class VerySecretRestService extends AbstractRestService {
	...
	@GET
	@Path("/{token}")
	public Response getSecret(@Context UriInfo uriInfo, @Context Response resp, @PathParam("token") String token)
	...
	public Response authorize(@QueryParam("code") String code,
			@QueryParam("state") String state, @Context Request request,
...
//		//
//		//
//		 System.out.println(Response.temporaryRedirect(uri).build());
		// return Response.temporaryRedirect(uri).build();
	...
	/*
	 * https://accounts.google.com/o/oauth2/v2/auth?client_id=670282150821-0
	 * kd0pjsm09re6mtvjdm5s9ejek3hc4a0
	 * .apps.googleusercontent.com&response_type=code&scope=openid%
	 * 20email&state=http://localhost:8080/rest/verysecret&acces_type=offline&redirect_uri=http%3A%2F%2Flocalhost:8080%2Frest%2Fverysecret
	 * 
	 * code=4/EDxem0UwFLAT2xY8eMeO9nmNO9z-f6rD7zlkxAj5g3o
	 */
}